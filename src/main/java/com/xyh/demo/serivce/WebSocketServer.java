    ...
    public void sendMessage(Session session, String message) throws IOException {
        if(session != null){
            synchronized (session) {
                if(message.indexOf("toUserId")>0){
                    MessageDto dto = JSONObject.parseObject(message, MessageDto.class);
                    sessionPools.get(dto.getToUserId()).getBasicRemote().sendText(message);
                }else {
